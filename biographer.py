...
with st.sidebar:
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("📝 Complete Profile", use_container_width=True, key="profile_btn"): 
    ...
    col1, col2 = st.columns(2)
    with col1:
        if st.button("🔒 Privacy", use_container_width=True, key="privacy_btn"):
            st.session_state.show_privacy_settings = True
            st.rerun()
    with col2:
        if st.button("🎨 Cover", use_container_width=True, key="cover_btn"):
    ...
    if st.button("📋 Bank Manager", use_container_width=True, type="primary", key="bank_mgr_btn"): 
        st.session_state.show_bank_manager = True
        st.rerun()
    if st.session_state.get('current_bank_name'): 
        st.info(f"**Current:** {st.session_state.current_bank_name[:30]}...")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.session_state.current_question_bank:
        session_options = [f"Session {s['id']}: {s['title'][:20]}..." for s in st.session_state.current_question_bank]
        ...
        if selected_idx != st.session_state.current_session:
            st.session_state.update(current_session=selected_idx, current_question=0, editing=False, current_question_override=None, show_ai_rewrite_menu=False)
            st.rerun()
        
        s = st.session_state.current_question_bank[st.session_state.current_session]
        sid = s["id"]
        sdata = st.session_state.responses.get(sid, {})
        answered = len(sdata.get("questions", {}))
        total = len(s["questions"])
        status_color = "🟢" if answered == total else "🟡" if answered > 0 else "🔴"
        st.caption(f"{status_color} Current: {answered}/{total} topics")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    # ===== VIGNETTES =====
    st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
    st.markdown("### ✨ Vignettes")
    col1, col2 = st.columns(2)
    with col1:
        if st.button("📝 New", use_container_width=True, key="vignette_new_btn"): 
            import uuid
            ...
            st.rerun()
    with col2:
        if st.button("📖 View All", use_container_width=True, key="vignette_view_btn"): 
    ...
    # ===== SESSION MANAGEMENT =====
    st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
    st.markdown("### 📋 Session Mgmt")
    col1, col2 = st.columns(2)
    with col1:
        if st.button("All Sessions", use_container_width=True, key="all_sessions_btn"): 
            st.session_state.show_session_manager = True
            st.rerun()
    with col2:
        if st.button("Custom", use_container_width=True, key="custom_session_btn"): 
            st.session_state.show_session_creator = True
            st.rerun()
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    if st.session_state.confirming_clear == "session":
        st.warning("**Delete current session?**")
        col1, col2 = st.columns(2)
        with col1:
            if st.button("✅ Yes", type="primary", key="conf_sesh", use_container_width=True): 
                ...
                st.rerun()
        with col2:
            if st.button("❌ No", key="can_sesh", use_container_width=True): 
                st.session_state.confirming_clear = None
                st.rerun()
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("✅ Yes, Delete All", type="primary", key="conf_all", use_container_width=True): 
                for s in SESSIONS:
                ...
                st.rerun()
        with col2:
            if st.button("❌ No, Cancel", key="can_all", use_container_width=True): 
                st.session_state.confirming_clear = None
                st.rerun()
    else:
        col1, col2 = st.columns(2)
        with col1:
            if st.button("🗑️ Clear Session", use_container_width=True, key="clear_session_btn"): 
    ...
    st.caption("⚠️ Deleted data cannot be recovered")
    st.markdown('</div>', unsafe_allow_html=True)
    
    # ===== FOOTER =====
    st.markdown('<div class="sidebar-footer">', unsafe_allow_html=True)
    st.markdown("Tell My Story v2.0")
    if st.session_state.user_account:
        profile = st.session_state.user_account['profile']
...
Looking at your script, the issue is likely in the sidebar code. After adding the Publish button, half the sidebar disappeared because the layout got cramped and elements are being cut off. Let me fix this by reorganizing the sidebar and making it scrollable:

python
# ============================================================================
# SIDEBAR - FIXED VERSION WITH SCROLLABLE CONTENT
# ============================================================================
with st.sidebar:
    # Use custom CSS to make sidebar scrollable
    st.markdown("""
    ...
    # Sessions - Make this collapsible to save space
    st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
    st.markdown("### 📖 Sessions")
    
    # Add a session selector dropdown to save space
    if st.session_state.current_question_bank:
        session_options = [f"Session {s['id']}: {s['title'][:20]}..." for s in st.session_state.current_question_bank]
        selected_idx = st.selectbox(
            "Jump to session:",
        ...
        # Show current session status compactly
        s = st.session_state.current_question_bank[st.session_state.current_session]
        sid = s["id"]
        sdata = st.session_state.responses.get(sid, {})
        answered = len(sdata.get("questions", {}))
    ...
    # Vignettes Section - Compact
    st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
    st.markdown("### ✨ Vignettes")
    
    # Use two rows for buttons to save vertical space
    col1, col2 = st.columns(2)
    with col1:
    ...
    # PUBLISH SECTION - MOVED UP AND MADE PROMINENT
    st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
    st.markdown("### 🖨️ Publish Your Book")
    ...
    # Data Management - Compact at bottom
    st.markdown('<div class="sidebar-section">', unsafe_allow_html=True)
    st.markdown("### ⚙️ Data")
    
    # Search
    search_query = st.text_input("🔍", placeholder="Search stories...", key="global_search", label_visibility="collapsed")
    if search_query and len(search_query) >= 2:
        results = search_all_answers(search_query)
        if results:
            with st.expander(f"📖 {len(results)} Results", expanded=False):
                for i, r in enumerate(results[:5]):
                    st.markdown(f"**{r['session_title']}**  \n{r['question'][:30]}...")
                    if st.button(f"Go", key=f"srch_{i}", use_container_width=True):
                        for idx, s in enumerate(SESSIONS):
                            if s["id"] == r['session_id']:
                                st.session_state.update(current_session=idx, current_question_override=r['question'], show_ai_rewrite_menu=False)
                                st.rerun()
                    st.divider()
                if len(results) > 5: 
                    st.caption(f"... {len(results)-5} more")
    ...
    if st.session_state.confirming_clear == "session":
        ...
        col1, col2 = st.columns(2)
        with col1:
            if st.button("✅ Yes All", type="primary", key="conf_all", use_container_width=True): 
                for s in SESSIONS:
                    st.session_state.responses[s["id"]]["questions"] = {}
                save_user_data(st.session_state.user_id, st.session_state.responses)
                st.session_state.confirming_clear = None
                st.rerun()
        with col2:
            if st.button("❌ No", key="can_all", use_container_width=True): 
                st.session_state.confirming_clear = None
                st.rerun()
    else:
        col1, col2 = st.columns(2)
        with col1:
            if st.button("🗑️ Session", use_container_width=True): 
                st.session_state.confirming_clear = "session"
                st.rerun()
        with col2:
    ...
    # Footer
    st.markdown("---")
    st.caption("Tell My Story v2.0")
The key fixes:

Added scrollable sidebar - Now you can scroll to see all content

Reorganized sections - Made everything more compact with columns

Moved Publish section up - Now prominently displayed near the top

Added collapsible elements - Search results collapse to save space

Used compact layouts - Two-column buttons throughout

Added session selector dropdown - Replaces long list of session buttons

Proper scrollbar styling - Makes it clear the sidebar is scrollable

This should solve the disappearing half-sidebar issue while keeping all functionality accessible!

ok can you do the same for this and at the botom there is supopose to be a search box and  a delete data bo xin fyou can find that# biographer.py – Tell My Story App (COMPLETE VERSION WITH AI REWRITE AND IMPORT)
import streamlit as st
import json
from datetime import datetime, date
from openai import OpenAI
