...
# ============================================================================
# PDF GENERATION FUNCTIONS - FIXED UNICODE
# ============================================================================
class PDF(FPDF):
    def header(self):
        if hasattr(st.session_state, 'book_title') and st.session_state.book_title and self.page_no() > 1:
            ...
            # Use ASCII only
            title = st.session_state.book_title.encode('latin-1', 'replace').decode('latin-1')
    ...
    def footer(self):
        self.set_y(-15)
        self.set_font('Arial', 'I', 8)
        # Use ASCII only - no special characters
...
def generate_pdf(book_title, author_name, stories, format_style, include_toc, include_dates):
    ...
    # Cover page - encode all text to latin-1, replacing problematic characters
    pdf.set_fill_color(102, 126, 234)
    pdf.rect(0, 0, 210, 297, 'F')
    pdf.set_text_color(255, 255, 255)
    
    # Convert text to safe ASCII/latin-1
    safe_title = book_title.encode('latin-1', 'replace').decode('latin-1')
    safe_author = author_name.encode('latin-1', 'replace').decode('latin-1')
    safe_date = datetime.now().strftime("%B %d, %Y").encode('latin-1', 'replace').decode('latin-1')
    
    pdf.set_font('Arial', 'B', 30)
    pdf.cell(0, 40, '', 0, 1)
    pdf.cell(0, 20, safe_title, 0, 1, 'C')
    pdf.set_font('Arial', '', 16)
    pdf.cell(0, 10, f'by {safe_author}', 0, 1, 'C')
    pdf.set_font('Arial', '', 12)
    pdf.cell(0, 10, f'Generated on {safe_date}', 0, 1, 'C')
    pdf.add_page()
        ...
        if isinstance(stories, list):
            ...
            for i, story in enumerate(stories, 1):
                ...
                if session_id != current_session:
                    ...
                    safe_session = session_title.encode('latin-1', 'replace').decode('latin-1')
                    pdf.set_font('Arial', 'B', 12)
                    pdf.cell(0, 8, safe_session, 0, 1)
                    current_session = session_id
                question = story.get('question', f'Story {i}')
                safe_question = question[:50].encode('latin-1', 'replace').decode('latin-1')
                if len(question) > 50:
                    safe_question += "..."
                pdf.set_font('Arial', '', 12)
                pdf.cell(10, 6, f'{i}.', 0, 0)
                pdf.cell(0, 6, safe_question, 0, 1)
        pdf.add_page()
    ...
    if isinstance(stories, list):
        ...
        for story in stories:
            ...
            if session_id != current_session:
                ...
                pdf.cell(0, 10, safe_session, 0, 1, 'L')
                pdf.ln(5)
                current_session = session_id
            ...
            if format_style == 'interview':
                pdf.set_font('Arial', 'B', 12)
                pdf.multi_cell(0, 6, f'Q: {safe_question}')
                pdf.ln(2)
                ...
                pdf.cell(0, 8, f'Chapter {story_counter}: {safe_question}', 0, 1)
                pdf.ln(4)
                pdf.set_font('Arial', '', 11)
                pdf.multi_cell(0, 6, safe_answer)
                ...
                if b64:
                    try:
                        ...
                        if caption:
                            ...
                            pdf.cell(0, 6, f'Caption: {safe_caption}', 0, 1, 'C')
                        pdf.ln(5)
                    except:
                        pass
            pdf.ln(5)
